window.Gauge = function ( uniqueId,
                          separatedLabels,
                          point,
                          showDonut,
                          startAngle,
                ...
                if( showAnimation === 'yes' ){
                    ...
                    gauge.on('draw', function(data) {
                        if(data.type === 'slice') {
                            ...
                            /** If this was not the first slice, we need to time the animation so that it uses the
                             * end sync event of the previous animation. */
                            if(data.index !== 0) {
                                animationDefinition['stroke-dashoffset'].begin = 'anim' + (data.index - 1) + '.end';
                            }

                            /** We need to set an initial value before the animation starts as we are not in guided
                             * mode which would do that for us. */
                            data.element.attr({
    ...
    const elementGauge = document.querySelector(`.elementor-element-${uniqueId}` );

    window.addEventListener('scroll', function() {
        visibleGauge (elementGauge);
    });
