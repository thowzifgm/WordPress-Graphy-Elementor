...
use Merkulove\GraphyElementor\Unity\Cache;
use Merkulove\GraphyElementor\Unity\Helper;
use Merkulove\GraphyElementor\Unity\Plugin;
use Merkulove\GraphyElementor\UninstallHelper;
use Merkulove\GraphyElementor\Unity\EnvatoItem;
...
final class Uninstall {
    ...
    private function remove_settings() {
        ...
        foreach ( wp_load_alloptions() as $option => $value ) {
            if ( strpos( $option, 'mdp_graphy_elementor' ) === 0 ) {
                $settings[] = $option;
            }
        }
    ...
    public function get_uninstall_mode() {
        ...
        if ( isset( $uninstall_settings[ 'mdp_graphy_elementor_migration_settings' ] ) && $uninstall_settings[ 'mdp_graphy_elementor_migration_settings' ] ) { // Default value.
            $uninstall_settings = [
